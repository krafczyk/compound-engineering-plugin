...
| [`ce-code-review`](./ce-code-review.md), [`ce-work`](./ce-work.md), [`lfg`](./lfg.md) | `fast_review_route` | Optional fast pre-clearance binding for selected `ce-code-review` reviewer roles. It references an existing routing profile; absent or `null` preserves the one-pass review path. See [Phased review convergence](#phased-review-convergence). |
...
# Root setting: profile must already exist under routing.profiles.
fast_review_route: { profile: economy, policy: prefer }
```
...
`fast_review_route` is a root scalar setting, not a `routing.roles` or `routing.classes` binding. Its exact shape is `{ profile: <existing-profile-name>, policy: prefer|require }` and the referenced profile must exist in the merged `routing.profiles` map. Project config replaces the complete global value; `fast_review_route: null` in project config explicitly disables a global binding, while removing the project key inherits it again.

When `ce-work` or `lfg` requests `ce-code-review`'s private fast-if-configured phase, a matching authenticated current-task route still takes precedence. Otherwise, an effective fast-review binding routes only the review-class roles the review already selected. It does not add reviewers or affect research, verification, classifier, or fixer roles. The later authoritative pass is a fresh complete-diff review using ordinary role/class routing, including role-specific exceptions.

Both phases send eligible fixes through the normal review-fixer implementation-class route; fixers do not inherit either reviewer profile. The fast phase repeats while retained blocker-directed fixes make verified material progress, without a round cap. A changed diff alone is not progress when the same blocker survives without evidence that its failure scope narrowed. A fast stall preserves its evidence and escalates to ordinary review. The authoritative phase repeats under the same progress rule, but an authoritative stall blocks the workflow instead of becoming a residual. An unresolved required review or fixer route blocks instead of being treated as green or entering the unavailable-review manual fallback.

Audit-policy green means the completed current review has no blocking P0/P1 findings, apart from preference-grade `settled_conflict` findings. P2/P3 findings remain eligible for fixes or the existing durable residual flow unless active instructions promote them. With no effective fast binding, callers use their existing single ordinary review, fix, and residual behavior without an additional authoritative round.
