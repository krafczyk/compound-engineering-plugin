...
Each candidate has a `harness` (`codex`, `claude`, `grok`, or `cursor`) and an optional `model`. Omitting `model` means that harness's configured default. Composer is a model family reached through Cursor, so it is written as `harness: cursor` plus `model: composer`; that bare alias selects the fixed Composer default without making Composer models valid for another harness. Keep CLI flags and commands out of config; the list describes the desired author, while `ce-work` starts from its qualified adapter recipe and can inspect the installed CLI's help/version when a compatible invocation has drifted.

The list is intentionally host-relative. In Codex, the example collapses an equivalent Codex default to native only when no distinct model, effort, or route was requested; an explicit different same-harness model remains distinct. `ce-work` records preflight-unavailable candidates and locks the first qualified recipient before egress. After dispatch, it can advance only after authoritative terminal, unintegrated failure or identity rejection plus adapter-isolated or owner-discarded retry proof, exact abandonment, and a fresh sanction for the next declared candidate.

On native OpenCode, `ce-work` freezes the complete selected implementation wave before packet assembly through host-owned `ce_task_prepare`. For typed `native` or `opencode` results it supplies only the opaque session/role/instance-bound handle to each `ce_task`; typed `external` results continue through CE Work's durable external controller rather than generic `ce_task`. External preparation exposes only source/binding comparison identifiers, and the controller independently resolves public configuration and blocks if they differ or are omitted. OpenCode origin comes from the parsed request host, not an optional flag. Converted or non-native OpenCode therefore cannot enter external execution; non-OpenCode hosts remain comparison-free. Candidate family follows the first ordinal. A native OpenCode preflight cannot continue to a later external adapter because its prior-attempt authority cannot be transferred; this is an explicit mixed-adapter capability gap rather than a skipped fallback. Snapshots, locks, candidates, permissions, and serving claims never travel in model tool arguments or public Python resolver requests. Opaque handles are process-local and a restart requires a newly authorized top-level preparation rather than recovery from model-visible state.
...
Before any repository material leaves the host, `ce-work` discloses and records the instruction/config source, fixed recipient and intermediaries, bounded unit material exposed, and which restrictions are adapter-enforced versus cooperative. Routed model processes must be credential-free or use authentication brokered outside their readable filesystem domain. A route requiring copied JSON tokens, ambient credentials, home/config roots, or another readable CLI store is unavailable before model dispatch; legacy staged-auth input is rejected rather than exposed to model-controlled tool descendants. The controller still creates isolated `HOME`, XDG, cache/data, and temp roots and rejects `.env*` material. The detached runner gives the adapter its job identity; the controller validates the actual runner metadata and exact adapter argv before the external CLI starts, so a shell prefix or substituted worker cannot egress under a valid unit authorization. It also binds a co-located Linux Landlock adapter and pinned interpreter paths/digests, ABI, the exact route executable identity/digest, and allowed filesystem roots into attempt and dispatch receipts. Exact staged-secret scanning of workspace and result bytes remains a terminalization defense for retained legacy state, but it does not replace the pre-dispatch credential boundary. The adapter cannot switch recipients, widen scope, push, open a PR, or choose fallback. If a required restriction cannot be enforced, the route is unavailable.

Each external unit starts from a clean recorded SHA in a detached linked worktree under `/tmp/compound-engineering-<effective-uid>/ce-work/<run-id>/`. The linked worktree alone is concurrency and accidental-mutation containment, **not a security sandbox**. External dispatch additionally requires Linux Landlock filesystem confinement: the recipient can read/write its workspace and attempt environment and read its narrow runtime/system roots, but cannot read known absolute paths in the real home or canonical checkout, enumerate those roots, access the linked Git common directory, or mutate controller results. Unsupported hosts or a missing/changed confinement adapter make the route unavailable. Synchronous native units still use the active checkout; `ce-work` does not create a temporary worktree for every unit. If the selected plan is the only dirty path, `ce-work` discloses and creates a plan-only checkpoint commit first. Any unrelated dirt or tracked `.env*` makes the external route unavailable.
...
Worker output becomes one complete synthetic transport commit, including tracked and untracked edits, binary files, deletes, renames, and mode changes. After the host inspects its actual scope, one fail-stop controller transaction first finalizes the locked route against adapter-owned model/effort evidence. Missing or mismatched required identity leaves output quarantined before canonical mutation. Accepted output then acquires the integration lock, revalidates the canonical checkout, applies without committing, runs authoritative tests, reconciles test side effects, creates one host-owned canonical commit, and records cleanup. A failed pre-commit step cannot fall through into a later commit; it restores the exact pre-fold checkout before another unit or fallback may start. Unknown canonical movement blocks integration.
...
Not by themselves. They isolate concurrent Git state and contain accidental mutation. Before an external CLI can receive a packet, `ce-work` additionally applies controller-bound Linux Landlock filesystem confinement. The recipient gets only its workspace, a secret-free isolated attempt environment, selected executable runtime, and required system files; real-home absolute paths, host-home enumeration, the canonical checkout, controller results, linked Git metadata, and staged credential files are denied. The Landlock supervisor remains the child subreaper through bounded TERM-to-KILL cleanup, and fallback or cleanup cannot advance without bound evidence that every dispatched descendant is gone. If that OS or authentication boundary cannot be established, the route is unavailable rather than weakened.
