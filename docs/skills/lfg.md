...
The first example is preference-strength. If the Codex route is unavailable, `ce-work` advances only to another declared candidate, an exact same-host default candidate, or explicit `ce-default`; an exhausted preference without one of those paths returns a blocker. `lfg` discloses every attempted route and retains one shipping tail. The second is requirement-strength: an unavailable or unverifiable required route blocks without asking or silently switching to native work.
...
When the prompt has no stage instruction, `lfg` passes no empty binding for that stage. For planning, `ce-plan` resolves generalized role routing and the compatible `plan_model` setting (or no elevation). For implementation, `ce-work` considers applicable session/project instructions before merged project/global role and implementation-class routes, then the narrower compatible `work_engine_mode` and ordered `work_engine_preferences` setting at each layer. Legacy `prefer` retains its explicit built-in fallback; generalized profiles require final `ce-default` when built-in fallback is intended. `require` blocks without prompting when no candidate qualifies or concrete identity cannot be verified.
