...
- The native CE package registers `ce_task_prepare` and `ce_task` because OpenCode's native Task tool has no per-call model/variant selector. The owning workflow prepares every homogeneous role group in its selected wave before mutation; dispatch-site labels and bare worker names are not roles. `native` and `opencode` preparation results pass only an opaque session/role/instance-bound handle to each routed task; `external` returns CE Work to its durable external controller instead of generic `ce_task`.
- The adapter is tested against `@opencode-ai/plugin` and `@opencode-ai/sdk` `1.18.9` plus the MkChad host prompt-resolution capability, including a completed live 92-role dispatch. At runtime it preflights observable session, agent, config, model, prompt, abort, and status APIs rather than treating an injected or package version string as server attestation.
...
- The MkChad host exposes native Task's prompt-part resolver through plugin tool context. Routed prompts therefore preserve `@file`, `@directory`, and `@agent` expansion without hand-parsing. Hosts without that capability fail such prompts unavailable before child creation rather than silently degrading them.
