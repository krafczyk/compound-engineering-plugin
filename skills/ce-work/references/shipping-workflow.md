...
3. **Code Review**
   ...
   **3a. Review and convergence (read-only review).** Pin the merge base, then first invoke `ce-code-review` with `mode:agent review_phase:fast-if-configured` (add `plan:<path>` when known; `base:<ref>` when resolved). Pass **`depth:full`** when the plan, task, or user explicitly asked for a full / deep / thorough review. Do not pass `mode:autofix`. Parse the JSON and load `references/review-convergence.md` before applying findings. When fast review is inactive, consume this first ordinary result through the exact legacy one-pass path below; do not run an authoritative extra review. When active, follow the reference through authoritative green or a blocked authoritative stall.

<!-- ce-dispatch-exclude:delegated-dispatch-reference -->
   **3b. Apply fixes (caller-owned).** For every active convergence round, load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix subagents using `ce-work.review-fixer`. Orchestrator merges, tests, and commits; report retained fix and required-verification evidence to the convergence state. On the inactive branch this is the unchanged legacy followup. Proceed to the Residual Work Gate only after authoritative green, or immediately after the legacy branch completes.

   **If `ce-code-review` cannot run at all** — subagent dispatch unavailable, unauthenticated, or hard-capped, returning `status: failed`/`degraded` with no coverage even after its own sequential Fallback: in an **interactive** session, run the harness-native review if one exists (e.g. `/review`) and fix inline; in a **non-interactive** session (autonomous pipeline, or no native review available), skip the dedicated step, note `Code review: skipped (ce-code-review unavailable)`, and add an explicit manual diff scan to Final Validation. This fallback does not apply when `blocking_route_failures` is non-empty; an unresolved required route blocks. Never silently ship a non-mechanical change with no review of any kind.
