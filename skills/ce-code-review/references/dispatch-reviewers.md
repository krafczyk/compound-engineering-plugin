...
Use the run ID and absolute run dir already created at the Stage 3d routing boundary. Pass `{run_id}` and `{run_dir}` to every persona sub-agent so they can write their full analysis to `{run_dir}/{reviewer_name}.json`. Native routing instance IDs must include both values as `<run_id>-<reviewer_name>` so a later convergence round in the same host session is a distinct selected wave while retries within one round retain their identity.
...
<!-- ce-dispatch-exclude:dispatch-permission-policy -->
...
<!-- ce-dispatch-exclude:batch-scheduling-policy -->
...
**Routing batch: `ce-code-review.local-reviewers`.** With Stage 3d's final existing local roster fixed and before any template or persona prompt is read or assembled, load `references/execution-routing.md` and resolve every selected stable ID from `ce-code-review.correctness-reviewer`, `ce-code-review.security-reviewer`, `ce-code-review.adversarial-reviewer`, `ce-code-review.project-standards-reviewer`, `ce-code-review.testing-reviewer`, `ce-code-review.maintainability-reviewer`, `ce-code-review.agent-native-reviewer`, `ce-code-review.performance-reviewer`, `ce-code-review.api-contract-reviewer`, `ce-code-review.data-migration-reviewer`, `ce-code-review.reliability-reviewer`, `ce-code-review.previous-comments-reviewer`, `ce-code-review.julik-frontend-races-reviewer`, `ce-code-review.swift-ios-reviewer`, `ce-code-review.learnings-researcher`, and `ce-code-review.deployment-verification-agent` together in one `ce-routing/v1` `resolve_batch` against one frozen snapshot. Omit every role not in the materialized local roster; in particular, a started peer keeps the in-process adversarial role absent.

When the private `review_phase:fast-if-configured` control is active, request `routing_phase: fast-review` for actual review-class roles only. `learnings-researcher` and `deployment-verification-agent` remain ordinary: classifiers, research/verification helpers, validators, and fixers never receive phase metadata. On OpenCode, group each selected review role for `ce_task_prepare` with `routing_phase: "fast-review"`, retain its opaque handle, and consume the returned `routing_phase.requested` / `routing_phase.active` values. On non-OpenCode hosts, add the phase only to review-role instance metadata and consume each resolution `routing_phase` state. Reuse one frozen snapshot, including the adversarial gate's snapshot when it ran first. The phase token and routing data never enter a template, persona asset, scope, or reviewer prompt. A requested fast phase is active when any selected review-class outcome reports active; role-specific task precedence may leave other reviewers ordinary, but the mixed result still requires the later complete-diff authoritative review. Record inactive only when every selected review-class outcome is ordinary.
...
<!-- ce-dispatch-site:ce-code-review.local-reviewers -->
...
After the inline fast pass has completed and the local reviewer batch has started, prepare synthesis inputs while reviewers run. Do not poll the peer during that wave. After local reviewers finish, if Stage 3d persisted a peer job ID, perform the reference's single bounded status/wait/reap sequence and fold in whatever terminal artifact is available. Attribute from the artifact and clean up through the runner. An unconfigured or policy-authorized additive failure or timeout stays non-blocking and is named in Coverage; a finalized `require` blocker enters `blocking_route_failures`. Neither outcome triggers a late in-process adversarial retry. Peer findings enter ordinary synthesis, but agreement promotion requires top-level `independence_verified: true`; false or absent independence is useful evidence, not different-model corroboration. Coverage must say whether the adversarial lens ran cross-model or used the in-process fallback.
