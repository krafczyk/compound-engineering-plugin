...
3. Fix the selected set as **one** deterministic validator batch, ordered by severity and stable `#`. Eight findings is the normal cap. When more than eight P0/P1 survive, expand that same batch past the normal cap to include every surviving P0/P1 rather than silently omitting a blocker; never split the work into another batch. Cost, elapsed time, confidence, or a finding appearing mechanically obvious never licenses an additional skip; when the required validator cannot run, keep P0/P1 as validation-degraded and say so.
**Routing batch: `ce-code-review.finding-validator`.** Once that exact validator wave is fixed and before `references/validator-batch-template.md` is read or its prompt is assembled, load `references/execution-routing.md` and resolve `ce-code-review.finding-validator` in one `ce-routing/v1` `resolve_batch`. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen binding on recovery. Routing cannot add, remove, split, reorder, or change the foreground execution of validator findings.
<!-- ce-dispatch-site:ce-code-review.finding-validator -->
4. Read `references/validator-batch-template.md`, assemble the fixed wave, and run the validator batch foreground with background execution off. The validator returns one independent verdict per finding. A foreground Agent call is the wait. Never use shell no-ops (`echo waiting`, `noop`, `yield turn`, `end turn`, `true`, or sleeps), scheduled wakeups, status/list calls, or narrated "still waiting" turns.
