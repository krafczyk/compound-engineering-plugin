...
**Routing batch: `ce-code-review.trivial-pr-classifier`.** After the closed/merged skip rule does not fire and the PR-state path reaches the existing triviality judgment, but before its inline prompt is assembled, load `references/execution-routing.md` and resolve `ce-code-review.trivial-pr-classifier` in one `ce-routing/v1` `resolve_batch` against one frozen snapshot. Routing cannot add this classifier to standalone, `base:`, or branch-remote paths.
